...
group.add_argument('--smi', type=str, help='Path to .smi file. Can contain both the SMILES string and name, or just SMILES string')
group.add_argument('--csv', type=str, help='Path to csv file with list of smiles and number conformers')
parser.add_argument('--model_dir', type=str, help='Path to folder with trained model and hyperparameters')
parser.add_argument('--ckpt', type=str, default='best_model.pt', help='Checkpoint to use inside the folder')
parser.add_argument('--out', type=str, help='Path to the output file. Takes both .smi and .csv.')
...
with open(f'model_parameters.yml') as f:
    args.__dict__.update(yaml.full_load(f))
args.batch_size = batch_size  # override the training one
if not args.no_model:
    model = get_model(args)
    state_dict = torch.load(f'best_model.pt', map_location=torch.device('cpu'))
    model.load_state_dict(state_dict, strict=True)
    model = model.to(device)
    model.eval()

if args.smi:
    test_data = pd.read_csv(args.smi, sep='\t').values

if args.csv:
    test_data = pd.read_csv(args.csv).values
...
if args.smi:
    #print('Inside smi')
    for idx, (smi, pdb) in test_data:
        if type(args.confs_per_mol) is int:
            mols = sample_confs(smi, args.confs_per_mol, smi)
        else:
            mols = sample_confs(smi, 20, smi)
        ...
        if not args.no_energy:
            ...
            print(
                f'{idx} rotable_bonds={mols[0].n_rotable_bonds} n_confs={len(rmsd)}',
            ...
            print(f'{idx} rotable_bonds={mols[0].n_rotable_bonds} n_confs={len(mols)}', smi, flush=True)
        if(pd.isna(pdb)):
            conformer_dict[idx] = mols
        else:
            conformer_dict[pdb] = mols

    print('Generated conformers for', len(conformer_dict), 'molecules')

    if args.out:
        file_name = f'{args.out}'
    else:
        file_name = 'pooled_molecules.sdf'

    with Chem.SDWriter(file_name) as w:
        for k,v in conformer_dict.items():
            for i in v:
                m = Chem.AddHs(i)
                ...
                if (m == None):
                    ...
                    index = "Index: " + str(k)
                    print(index + ' added to file' )
                    m.SetProp('_Name', index)
                    w.write(m)

if args.csv:
    for smi_idx, (raw_smi, n_confs, smi, path) in test_data:
        if type(args.confs_per_mol) is int:
        ...
        conformer_dict[path] = mols

    if args.out:
        file_name = f'{args.out}'
    else:
        file_name = 'conformers.pkl'
